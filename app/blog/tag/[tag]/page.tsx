import { getPosts, getPostTags } from '../../../lib/contentful';
...
export async function generateStaticParams() {
  ...
  return (tags as string[]).map((tag) => ({ tag }));
}
...
export default async function TagPage({ params }: { params: { tag: string } }) {
  ...
  const posts = await getPosts({ tag: decodedTag });
  const allTags = await getPostTags();
  
  if (!(allTags as string[]).includes(decodedTag)) {
    return notFound();
  }
      ...
      <section className="relative flex flex-col pt-20 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-base lg:text-lg text-slate-600 max-w-2xl mx-auto font-light">
              Browse articles related to {decodedTag} by Dr. Dashrath R Basannar
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 lg:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10 flex flex-wrap gap-2 justify-center">
            <Link 
              href="/blog" 
              className="px-4 py-2 rounded-full text-sm font-medium bg-slate-100 text-slate-800 hover:bg-teal-50 hover:text-teal-700 transition-colors"
            >
              All
            </Link>
              ...
              <Link 
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium ${
                  t === decodedTag 
                    ...
                    : 'bg-slate-100 text-slate-800 hover:bg-teal-50 hover:text-teal-700'
                } transition-colors`}
              >
                {t}
              </Link>
          ...
          {posts.length === 0 && (
            <div className="text-center py-20">
              <h3 className="text-xl font-medium text-slate-800">No posts found with tag: {decodedTag}</h3>
              <p className="text-slate-600 mt-2">Try another tag or check back soon for new content!</p>
            </div>
          )}
        </div>
      </section>
